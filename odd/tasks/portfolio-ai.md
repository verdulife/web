...
### Conversation persistence (R1) — done

The site is static (Astro): navigating to /sobre-mi is a full page load and module state dies with it. The conversation survives navigation **within the tab** via `sessionStorage` (per-tab, cleared when the tab closes) — the storage key is `portfolio-chat-state-v1` and the shape is `{ v: 1, messages: [{ role, content, widgets? }], scrollRatio }`.

- **messages + widgets + scrollRatio**: assistant entries persist with their `widgets` descriptors (sanitized: plain objects with numeric `index` + string `type`, capped at 6/message); `scrollRatio` is the thread's scroll position over its scrollable range, clamped to `[0, 1]` (0 when not scrollable, NaN/negative → 0). Load validates envelope (`v === 1`, `messages` array, non-empty string content, role user|assistant) and caps to the last 30 messages; any corrupt/unusable state is removed and falls back to the fresh-intro path.
- **Save points**: after each completed user-visible turn (the user message right after it is appended — it survives even if the request never answers — and the assistant message once its turn has fully mounted) plus `pagehide` (navigation, reload, back/forward). All sessionStorage access is try/caught: quota/private-mode failures are silent no-ops.
- **Restore is instant**: when state exists, `init()` replaces `streamIntro()` with the restore path — greeting rendered as a plain first turn (no typewriter), every message mounted through the shared paragraph lifecycle (text paragraphs + inline widgets inside, block widgets as flow siblings, no empty paragraphs), suggestions at the bottom, saved scroll ratio applied via `requestAnimationFrame` once layout settles, composer re-enabled and input focused. The streaming path (`streamSegments`) and the restore path share one paragraph-lifecycle core, so a restored turn is byte-identical to a streamed one.
- **R1 evidence (2026-09-23)**: `node --check public/scripts/conversation.js` → OK; `bun run check` (root) → exit 0, 0 errors/0 warnings/0 hints; DOM-stub + fake-sessionStorage harness (scratch, outside the repo) 9/9 scenarios: (a) after one user+assistant turn the stored history is exactly `[user, assistant]` (intro is NOT in history; the intro is rendered, not persisted; restore shows the 3 DOM turns intro+user+assistant); (b) widget round trip — a reply with an inline link + block image saves its descriptors, and reloading restores the block card between paragraphs with zero timers and byte-identical DOM; (c) scrollRatio save/restore (0.5 → 250px) plus guards (not-scrollable/negative/NaN → 0, over-1 → 1, corrupt persisted ratio → 0); (d) corrupt JSON / wrong `v` / missing `messages` / unusable entries → fresh intro path and state removed; (e) 35 messages capped to the last 30 on load and on re-save; (f) a subsequent ask continues from the restored history (request payload includes restored messages, state grows to 4); (g) broken storage (setItem throws) degrades silently; (h) the non-reduced typewriter path still drives turns to the same DOM (regression guard on the shared lifecycle refactor).
## Delivery (2026-09-22)
